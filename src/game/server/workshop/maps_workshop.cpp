		...
		if ( engine->IsDedicatedServer() && i )
		{
			...
			if ( pUGCPath )
			{
				...
				if ( *szFullPath )
				{
					...
					pUGC->BInitWorkshopForGameServer( 265630, szFullPath );
				}
				else
				{
...
void CTFWorkshopMap::Steam_OnQueryUGCDetails( SteamUGCQueryCompleted_t *pResult, bool bError )
{
	...
	char szWorkshopId[128];
	Q_snprintf( szWorkshopId, sizeof( szWorkshopId ), "%u", m_nFileID );

	CUtlString baseName = CUtlString( szWorkshopId );
...
bool CTFWorkshopMap::GetLocalPath( /* out */ CUtlString &strLocalPath )
{
	uint64 nUGCSize = 0;
	uint32 nTimestamp = 0;
	...
	Q_snprintf( szDirectory, sizeof( szDirectory ), "%s/maps/*.bsp", szFolder );
	const char *pMatFileName = g_pFullFileSystem->FindFirst( szDirectory, &mapHandle );

	if ( !pMatFileName || pMatFileName[0] == '\0' )
		return false;
	...
	if ( pMatFileName )
		Q_strncpy( szFileName, pMatFileName, sizeof(szFileName) );

	char szLocalDirectory[_MAX_PATH];
	Q_snprintf( szLocalDirectory, sizeof( szLocalDirectory ), "maps/%s", szFileName );

	V_MakeAbsolutePath( szFilePath, sizeof( szFilePath ), szLocalDirectory, szFolder );
...
bool CTFMapsWorkshop::CanonicalNameForMap( PublishedFileId_t fileID, const CUtlString &originalFileName, /* out */ CUtlString &strCanonName )
{
	...
	int len = strCanonName.Format( "workshop/%llu", fileID );
	if ( len >= MAX_PATH )
	{
		Assert( len < MAX_PATH );
