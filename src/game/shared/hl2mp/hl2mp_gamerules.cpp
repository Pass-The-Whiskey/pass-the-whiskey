...
ConVar fof_sv_enablecurrency( "fof_sv_enablecurrency", "1", FCVAR_REPLICATED );

ConVar fof_sv_currentmode( "fof_sv_currentmode", "1", FCVAR_REPLICATED );
ConVar fof_sv_weaponmenu( "fof_sv_weaponmenu", "1", FCVAR_REPLICATED );
ConVar fof_sv_maxteams( "fof_sv_maxteams", "4", FCVAR_REPLICATED );
...
BEGIN_NETWORK_TABLE_NOBASE( CHL2MPRules, DT_HL2MPRules )
		...
		RecvPropInt( RECVINFO( m_iCurrentTeamCount ) ),
		RecvPropInt( RECVINFO( m_iCurrentGameMode ) ),
	#else
		SendPropBool( SENDINFO( m_bTeamPlayEnabled ) ),

		SendPropInt( SENDINFO( m_iCurrentTeamCount ) ),
...
CHL2MPRules::CHL2MPRules()
{
	...
	m_iCurrentTeamCount = fof_sv_maxteams.GetInt();
	m_iCurrentGameMode = fof_sv_currentmode.GetInt();
...
//	float flTimeLimit = mp_timelimit.GetFloat() * 60;
	...
	if ( flFragLimit )
	{
		if( IsTeamplay() == true )
		{
			CTeam *pCombine = g_Teams[TEAM_VIGILANTES];
			CTeam *pRebels = g_Teams[TEAM_DESPERADOS];
...
{  
	// kick scale: 0.1893939

	return (pPlayer->m_Local.m_flFallVelocity - 315.9) * 0.1136364;
}
...
void CHL2MPRules::ClientSettingsChanged( CBasePlayer *pPlayer )
{
	...
	if ( stricmp( szModelName, pCurrentModel ) )
	{
		...
		else
		{
			...
			{
				pHL2Player->ChangeTeam( TEAM_DESPERADOS );
			}
			else
			{
				pHL2Player->ChangeTeam( TEAM_VIGILANTES );
			}
		}
	}
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("AR2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("AR2AltFire",		DMG_DISSOLVE,				TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,							0 );
		def.AddAmmoType("Pistol",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("Revolver",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(800, 5000),	0 );
		def.AddAmmoType("SMG1",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("SMG1_Grenade",		DMG_BURN,					TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,							0 );
		def.AddAmmoType("Arrow",			DMG_BULLET,					TRACER_LINE,			0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(800, 8000),	0 );
		def.AddAmmoType("Buckshot",			DMG_BULLET | DMG_BUCKSHOT,	TRACER_LINE,			0,			0,			INFINITE_AMMO,			BULLET_IMPULSE(400, 1200),	0 );
		def.AddAmmoType("Rocket",			DMG_BURN,					TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,							0 );
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,							0 );
		def.AddAmmoType("Slam",				DMG_BURN,					TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,							0 );
	
		def.AddAmmoType("Rifle",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	60,			60,			INFINITE_AMMO,			0x4426fe46,	4,	6,		8);
		def.AddAmmoType("Knife",			DMG_SLASH,					TRACER_NONE,			25,			25,			50,						0x42a395d1,	5,	8,		8);
		def.AddAmmoType("Axe",				DMG_SLASH,					TRACER_NONE,			40,			40,			50,						0x4308522e, 8,	0xc,	8);
		def.AddAmmoType("Hammer",			DMG_CLUB,					TRACER_NONE,			40,			40,			50,						0x4308522e, 8,	0xc,	8);
		def.AddAmmoType("Machete",			DMG_SLASH,					TRACER_NONE,			40,			40,			50,						0x4308522e,	8,	0xc,	8);
		def.AddAmmoType("RockSalt",			DMG_BURN | DMG_NEVERGIB,	TRACER_LINE_AND_WHIZ,	4,			4,			INFINITE_AMMO,			BULLET_IMPULSE(800, 5000),	3,	6,		8);
		def.AddAmmoType("Dynamite",			DMG_BURN,					TRACER_NONE,			0,			0,			INFINITE_AMMO,			0,			0,	4,		8);
		def.AddAmmoType("Gatling",			DMG_BULLET,					TRACER_LINE,			45,			45,			INFINITE_AMMO,			0x4535e743,	0,	4,		8);
	}
